//
//  AddOccasionOfferViewController.swift
//  Aklaty
//
...
import Gallery
class AddOccasionOfferViewController: UIViewController {
    @IBOutlet weak var offerDiscriptionTextField: UITextField!
    @IBOutlet weak var discountRateTextField: UITextField!
    @IBOutlet weak var vaildTimeTextField: UITextField!
    @IBOutlet weak var doneButton: UIButton!
    //MARK: - Vars
   var addItemViewModel = AddNewOccasionOffer()
    ...
    override func viewDidLoad() {
        ...
        addItemViewModel.bindOccasionOfferViewModelToView = { [weak self] in
    ...
    func addNewItem(){
        ...
        offer.id = id
        offer.discountRate = Int(discountRateTextField.text ?? "")
        offer.offerDiscription = offerDiscriptionTextField.text
        offer.vaildTime = vaildTimeTextField.text
        offer.imageLinks = []
        ...
        if categoryItemImages.count > 0 {//then user add images
            uploadImages(images: categoryItemImages, itemId: id) { [ weak self] (imageLinkArray) in
                ...
                offer.imageLinks = imageLinkArray
                self?.addItemViewModel.addOccasionOfferToFirebase(offer: offer)
    ...
    private func textFieldsHaveText() -> Bool {
       return (offerDiscriptionTextField.text != "" && discountRateTextField.text != "" && vaildTimeTextField.text != "" )
...
extension AddOccasionOfferViewController: GalleryControllerDelegate {
