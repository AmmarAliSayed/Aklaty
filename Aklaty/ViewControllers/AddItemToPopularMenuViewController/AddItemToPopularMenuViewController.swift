//
//  AddItemToPopularMenuViewController.swift
//  Aklaty
//
...
class AddItemToPopularMenuViewController: UIViewController {
    ...
    func addNewItem(){
        ...
        if categoryItemImages.count > 0 {//then user add images
            uploadImages(images: categoryItemImages, itemId: id) { [ weak self] (imageLinkArray) in
                ...
                self?.addItemViewModel.addItemToPopularMenuFirebase(item: item)
...
extension AddItemToPopularMenuViewController: GalleryControllerDelegate {
