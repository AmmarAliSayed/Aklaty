//
//  AddItemToHealthyFoodViewController.swift
//  Aklaty
//
...
import Gallery
class AddItemToHealthyFoodViewController: UIViewController {
   ...
   var addItemViewModel = AddItemViewModel()
    ...
    override func viewDidLoad() {
        ...
        addItemViewModel.bindAddItemViewModelToView = { [weak self] in
    ...
    func addNewItem(){
        ...
        item.id = id
        item.name = foodNameTextField.text
        item.description = foodDescriptionTextField.text
        item.calories = Int(noOfCaloriesTextField.text ?? "")
        ...
        if categoryItemImages.count > 0 {//then user add images
            uploadImages(images: categoryItemImages, itemId: id) { [ weak self] (imageLinkArray) in
                ...
                self?.addItemViewModel.addItemToHealthyFoodFirebase(item: item)
...
extension AddItemToHealthyFoodViewController: GalleryControllerDelegate {
