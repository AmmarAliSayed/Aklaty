//
//  HealthyFoodItemDetailsViewModel.swift
//  Aklaty
//
...
import Foundation
import FirebaseAuth
class HealthyFoodItemDetailsViewModel: NSObject {
    //property from model
   ...
   var selectedItem :Item!
    ...
    func fetchItemDataFromApi(){
        if let  id = selectedItem.id {
            fireStoreService.retrieveHealthyFoodItemDetailsFromFirebase(id) { [weak self] (item) in
                self?.itemData = item
        }
        }else{
            print("error")
    ...
    func addOrderedHealthyFoodToFirebase(item : OrderedItem){
        fireStoreService.saveOrderedHealthyFoodItemToFirestore(item) { (error) in
    ...
    func saveOrderedHealthyFoodItemToBasket(item : OrderedItem){
        if Auth.auth().currentUser != nil {
            ...
            fireStoreService.downloadBasketFromFirestore(Auth.auth().currentUser!.uid) { (basket) in
                ...
                if basket == nil {
                    ...
                    basket!.healthyFoodIds.append(item.id)
                    self.updateBasket(basket: basket!, withValues: [ K.BasketFStore.healthyFoodIds  : basket!.healthyFoodIds])
                }
            }
        }else{
            //
    ...
    private func createNewBasket() {
        ...
        if let itemId = itemData.id  {
            newBasket.healthyFoodIds  = [itemId]
