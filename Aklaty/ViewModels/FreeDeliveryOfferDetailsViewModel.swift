//
//  FreeDeliveryOfferDetailsViewModel.swift
//  Aklaty
//
...
import Foundation
import FirebaseAuth
class FreeDeliveryOfferDetailsViewModel: NSObject {
   ...
   var selectedItem :Offer!
    ...
    func fetchOfferDataFromApi(){
        if let  id = selectedItem.id {
            fireStoreService.retrieveFreeDeliveryOfferDetailsFromFirebase(id) { [weak self] (item) in
                self?.itemData = item
        }
        }else{
            print("error")
    ...
    func addOrderedFreeDeliveryItemToFirebase(item : OrderedOffer){
        fireStoreService.saveOrderedfreeDeliveryItemToFirestore(item) { (error) in
    ...
    func saveFreeDeliveryItemToBasket(item : OrderedOffer){
        if Auth.auth().currentUser != nil {
            ...
            fireStoreService.downloadBasketFromFirestore(Auth.auth().currentUser!.uid) { (basket) in
                ...
                if basket == nil {
                    ...
                    basket!.FreeDeliveryIds.append(item.id)
                    self.updateBasket(basket: basket!, withValues: [ K.BasketFStore.FreeDeliveryIds  : basket!.FreeDeliveryIds])
                }
            }
        }else{
            //
    ...
    private func createNewBasket() {
        ...
        if let itemId = itemData.id  {
            newBasket.FreeDeliveryIds  = [itemId]
